...
main = do
  ...
  let today    = utctDay now
      ...
      defTo    = showDay today
      useTerminal = "--terminal" `elem` args
      dateArgs    = filter (/= "--terminal") args
  ...
  case (parseDay fromStr, parseDay toStr) of
    (Just fromDay, Just toDay) | fromDay < toDay -> do
      ...
      case histResult of
        ...
        Right entries -> do
          if useTerminal
            then terminalRender curUsd curEur curGbp curVol isHourly entries
            else do
    ...
    _ -> putStrLn "Error: invalid or reversed date range. Use: btcgui [--terminal] [YYYY-MM-DD YYYY-MM-DD]"
...
generateHtml curUsd curEur curGbp curVol isHourly entries =
  ...
  ++ "var chunk=ohlc.filter(function(c){return parseInt(c.timestamp)<toSec;})"
  ++ ".map(function(c){var date=new Date(parseInt(c.timestamp)*1000).toISOString().slice(0,10);"
  ++ "var price=parseFloat(c.close);var vol=parseFloat(c.volume)*price;"
  ...
  ++ "if(nextFrom>=toSec){onDone(all);}else{fetchBitstampChunks(nextFrom,toSec,all,onDone);}})"
  ...
  ++ "if(fromSec>=cutoffSec){document.getElementById('status').textContent='CoinGecko failed, trying Binance...';"
  ++ "fetchBinanceChunks(fromMs,toMs,[]);return;}"
  ...
  ++ "if(bsData.length===0){document.getElementById('status').textContent='No data available';"
  ++ "document.getElementById('fetchBtn').disabled=false;}"
  ++ "else{showDaily(bsData);document.getElementById('status').textContent='(via Bitstamp)';"
  ++ "document.getElementById('fetchBtn').disabled=false;}}"
  ++ "else{document.getElementById('status').textContent='Fetching from Binance...';"
