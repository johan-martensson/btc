...
generateHtml curUsd curEur curGbp curVol entries =
  ...
  ++ "var support=[],fair=[],resist=[];"
  ++ "labels.forEach(function(ds){"
  ++ "var days=dateToDays(ds);"
  ++ "if(days<=0){support.push(null);fair.push(null);resist.push(null);return;}"
  ...
  ++ "return {support:support,fair:fair,resist:resist};}"
  ...
  ++ "return {responsive:true,interaction:{mode:'nearest',intersect:false},"
  ++ "plugins:{legend:{labels:{color:'#ccc',font:{size:14}}},"
  ++ "tooltip:{backgroundColor:'#1e1e3a',titleColor:'#f7931a',bodyColor:'#e0e0e0',"
  ++ "borderColor:'#f7931a',borderWidth:1,padding:12,"
  ...
  ++ "if(!items.length)return '';"
  ++ "var days=items[0].raw.x;if(days<=0)return '';"
  ++ "var ld=Math.log10(days);"
  ++ "var fair=Math.pow(10,-17.01+5.82*ld);"
  ++ "var sup=Math.pow(10,-17.51+5.82*ld);"
  ++ "var res=Math.pow(10,-16.51+5.82*ld);"
  ++ "var f=function(v){return '$'+v.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})};"
  ++ "return ['','  PL Fair Value: '+f(fair),'  PL Support: '+f(sup),'  PL Resistance: '+f(res)];"
  ++ "}}}},"
  ...
  ++ "var datasets=[{label:'BTC Price',data:data,type:'line',yAxisID:'y',"
  ...
  ++ "if(vols&&vols.length>0){datasets.push({label:'Volume',data:vols,type:'bar',yAxisID:'y1',"
  ++ "backgroundColor:'rgba(78,186,234,0.25)',borderColor:'rgba(78,186,234,0.4)',"
  ++ "borderWidth:1,order:2});}"
  ++ "if(showPL){var pl=calcPowerLaw(labels);"
  ...
  ++ "priceChart=new Chart(ctx,{type:'line',data:{labels:labels,datasets:datasets},"
  ...
  ++ "if(showPL){var pl=calcPowerLawXY(labels);"
